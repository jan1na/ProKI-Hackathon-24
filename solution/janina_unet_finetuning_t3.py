...
# Force CPU usage, since T3 instances do not have a dedicated GPU
physical_devices = tf.config.list_physical_devices('CPU')
if physical_devices:
    try:
        # Set memory growth or other configurations if required for CPU (usually not needed)
        tf.config.set_logical_device_configuration(physical_devices[0], [tf.config.LogicalDeviceConfiguration(memory_limit=4096)])
        print(f"Using CPU: {physical_devices[0].name}")
    except RuntimeError as e:
        print("Error in setting device configurations:", e)
else:
    print("No CPU found.")
